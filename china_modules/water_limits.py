...
def define_components(mod):
    mod.WATER_BASIN_PERIODS = Set(dimen=2, validate=lambda m, b, p: p in m.PERIODS)
    ...
    mod.consistent_water_basin_names = BuildCheck(
        mod.WATER_BASINS,
        rule=lambda m, wb: len(
            set(
                [
                    m._water_basin_name_cn_raw[wb, p]
                    for _wb, p in m.WATER_BASIN_PERIODS
                    if _wb == wb
                ]
            )
        )
    ...
    mod.water_basin_name_cn = Param(
        mod.WATER_BASINS, initialize=default_water_basin_name_cn
    )
    ...
    normalized_dat = [
        {
            ...
            "AnnualCoolingWaterWithdrawals_mm3": value(
                m.AnnualCoolingWaterWithdrawals_mm3[wb, p]
            ),
        }
