...
"""
This version forces suspension of any retrofitted base gens capacity. This is
necessary to avoid retrofitting lots of capacity in the early years, then applying
it to different generators (or doubling up multiple retrofits) in later years as
the base gens age out. It also makes it clear which capacity is online at any given
time. And it forces faster amortization of retrofits based on short life of
existing gen and avoids carrying retrofits past the life of existing gen into new
plants of same type.
- this is better than the method with the commitment rules, because plants normally
wouldn't want to swap between modes (why build a mode if you won't use it?). This
also forces clear designation of how much capacity of each type exists, and forces
accelerated recovery of retrofit costs, which probably affects decisions

TODO: report total unavailable capacity (due to base retirements) somewhere, possibly in gen_cap.csv, possibly just as a reduction in SuspendGen

TODO: more generally, it would be nice to be able to affect GenCapacity from other modules, e.g. declare retrofitted and unavailable capacity in this module and have them reduce GenCapacity (probably per vintage first, then in aggregate), with a rule that GenCaoacity per vintage must never be negative. That would simplify the code (no need to check for built capacity to suspend) and allow decoupling (e.g. separate suspend module from build) and also clarify special states in the output files (can't currently distinguish between unavailable retrofit capacity and early retired, and have to jump through hoops to dustinguish retrofitted base gens from early retired ones.) This may require making a list of capacity adjustments and then creating GenCapacity in commit or dispatch module (or just automatically apply UnavailableGen and SuspendGen and if found).
