...
"""
Defines generation project construction plan.

This version is the same as generators.core.build, but allows suspending
operation for one or more periods to avoid fixed O&M.

NOTE: using permanent retirements (gen_build_early_retirement.py) slows down
solutions by a factor of 10 or more (mainly in the crossover stage?), so this
is an attempt to get similar results in a simpler, faster formulation.

TODO: report SuspendGen in gen_cap.csv
